...
-(void)showAlertViewWithCompleteBlock:(CompleteBlock)block dismissAfter:(NSTimeInterval)delay
{
    [self showAlertViewWithCompleteBlock:block];
    [NSTimer scheduledTimerWithTimeInterval:delay target:self selector:@selector(dismissAlert) userInfo:nil repeats:NO];
}
-(void)dismissAlert
{
    __block __weak UIAlertView *selfp = self;
    dispatch_async(dispatch_get_main_queue(), ^{
        [selfp dismissWithClickedButtonIndex:0 animated:YES];
    });
}
@end
